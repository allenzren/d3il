...
class BesoAgent(BaseAgent):
    ...
    def train_agent_on_epochs(self, train_loader, test_loader, epochs):
        ...
        for epoch in tqdm(range(epochs)):
            ...
            interrupt_training, best_test_mse = self.early_stopping(
                best_test_mse, mean_mse, self.patience, epochs
            )
            ...
            for batch in train_loader:
                ...
                wandb.log(
                    {"training/loss": batch_loss, "training/test_loss": avrg_test_mse}
                )
            wandb.log(
                {
            ...
            log.info(
                "Epoch {}: Mean train batch loss mse is {}".format(
                    epoch, np.mean(batch_losses)
                )
            )
    ...
    def train_agent_on_steps(
        ...
        for step in tqdm(range(self.max_train_steps)):
            ...
            wandb.log({"loss": batch_loss, "test_loss": avrg_test_mse})
    ...
    def predict(
        ...
        """
        noise_scheduler = (
            self.noise_scheduler if noise_scheduler is None else noise_scheduler
        )
        ...
        if self.window_size > 1:
            ...
            if get_mean is not None:
                x = (
                    torch.randn(
                        (len(input_state) * get_mean, 1, self.scaler.y_bounds.shape[1]),
                        device=self.device,
                    )
                    * self.sigma_max
                )
                ...
                x = (
                    torch.randn(
                        (len(input_state), 1, self.scaler.y_bounds.shape[1]),
                        device=self.device,
                    )
                    * self.sigma_max
                )
            ...
            if get_mean is not None:
                x = (
                    torch.randn(
                        (len(input_state) * get_mean, self.scaler.y_bounds.shape[1]),
                        device=self.device,
                    )
                    * self.sigma_max
                )
    ...
    def visualize_ode(
        ...
        if self.window_size > 1:
            ...
            self.obs_context.append(
                state
            )  # this automatically manages the number of allowed observations
            input_state = torch.stack(tuple(self.obs_context), dim=1)
        else:
            input_state = state
        ...
        for i in range(n_sampling_steps):
            ...
            x_0 = sample_ddim(
                self.model, state_rpt, x_0, goal_rpt, simgas_2, disable=True
            )
