...
from agents.models.beso.agents.diffusion_agents.k_diffusion.score_gpts import (
    DiffusionGPT,
)
...
class BesoAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.model.min_action = torch.from_numpy(self.scaler.y_bounds[0, :]).to(
            self.device
        )
        self.model.max_action = torch.from_numpy(self.scaler.y_bounds[1, :]).to(
            self.device
        )
        ...
        for num_epoch in tqdm(range(self.epoch)):
            ...
            if not (num_epoch + 1) % self.eval_every_n_epochs:
                ...
                if avrg_test_mse < best_test_mse:
                    ...
                    wandb.log(
                        {"best_test_mse": best_test_mse, "best_model_epochs": num_epoch}
                    )
